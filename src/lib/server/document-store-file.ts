...
export async function listDocumentsFile(): Promise<StoredDocument[]> {
...
export async function deleteDocumentFile(id: string): Promise<boolean> {
  const db = await readDb();
...
export async function listCommentsFile(
  documentId: string,
...
export async function addCommentFile(
  documentId: string,
  content: string,
  author = "You",
): Promise<StoredComment> {
